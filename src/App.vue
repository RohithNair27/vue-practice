...
export default {
    ...
    methods:{
        ...
        this.PlanesData=this.orginalData.filter((element)=>{
          let textSearched = element.model.toLowerCase().includes(this.inputText.toLocaleLowerCase()) 
...
<nav class="navbar bg-body-tertiary">
  <div class="container">
    <a class="navbar-brand" href="#">
      <img src='./assets/plane-svgrepo-com.svg' alt="Bootstrap" width="80" height="80">
      <h1>Plane tracker</h1>
    </a>
  </div>
</nav>
 <section class="input-container">           
<input v-model="inputText" placeholder="search for model" type="text" @input="event=> inputText=event.target.value"/>
<select name="manufacturer" class="dropdown" v-model="selected" >
    <option disabled value="">Please select manufacturer</option>
  <option v-for="plane in orginalData" :value="plane.manufacturer">{{ plane.manufacturer }}</option>
</select>
...
<button type="button" class="btn btn-primary btn-lg" @click="onFilter">Filter</button>
<button @click="onCall" class="btn btn-danger" type="button">Reset</button>
        </section>
